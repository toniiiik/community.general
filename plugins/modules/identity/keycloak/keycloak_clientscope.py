...
options:
    ...
    description:
        description:
            - Description of the client scope in Keycloak

    protocol:
        description:
            - Type of client scope (either C(openid-connect) or C(saml).
        choices: ['openid-connect', 'saml']

    protocol_mappers:
        description:
            - a list of dicts defining protocol mappers for this client scope.
    ...
    attributes:
        description:
            - A dict of further attributes for this client scope. This can contain various
...
def main():
    ...
    # See whether the client scope already exists in Keycloak
    if cid is None:
        before_clientt = kc.get_client_scope_by_name(module.params.get('name'), realm=realm)
        if before_clientt is not None:
            cid = before_clientt['id']
    else:
        before_clientt = kc.get_client_scope_by_id(cid, realm=realm)
    ...
    # If the client scope does not exist yet, before_client is still empty
    if before_clientt == dict():
        if state == 'absent':
            # do nothing and exit
            if module._diff:
                result['diff'] = dict(before='', after='')
            result['msg'] = 'client scope does not exist, doing nothing.'
        ...
        kc.create_client_scope(updated_clientt, realm=realm)
        after_clientt = kc.get_client_scope_by_name(updated_clientt['name'], realm=realm)

        result['end_state'] = after_clientt

        result['msg'] = 'client scope %s has been created.' % updated_clientt['name']
        module.exit_json(**result)
    else:
        if state == 'present':
            ...
            result['changed'] = True
            if module.check_mode:
                # We can only compare the current client scope with the proposed updates we have
                if module._diff:
            ...
            kc.update_client_scope(cid, updated_clientt, realm=realm)

            after_clientt = kc.get_client_scope_by_id(cid, realm=realm)
            ...
            result['msg'] = 'client scope %s has been updated.' % updated_clientt['name']
            module.exit_json(**result)
        else:
            ...
            kc.delete_client_scope(cid, realm=realm)
            result['proposed'] = dict()
            result['end_state'] = dict()
            result['msg'] = 'client scope %s has been deleted.' % before_clientt['name']
            module.exit_json(**result)
